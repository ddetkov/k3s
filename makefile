...
### init
k3s.init:
	@ssh ${USER}@${ENTRYPOINT} "sudo INSTALL_K3S_CHANNEL=stable INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_EXEC='server --cluster-cidr=172.30.0.0/16 --service-cidr=172.40.0.0/16 --data-dir=/data/k3s --disable=traefik,servicelb,metrics-server 
	 container-log-max-files=3 --kubelet-arg container-log-max-size=10Mi' /var/lib/rancher/k3s/install.sh"
